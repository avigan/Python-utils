...
def focal_ratio_fit(img, xthreshold=None, ythreshold=0.001, wave=None, pixel=None, center=True, rebin=2,
                    background_fit=True, background_fit_order=2, disp=False, ymin=1e-4):
    '''
    Compute the focal ratio from a PSF image using MTF = |OTF|
    ...
    def sampling_optim(rmax, pupil, dim, otf_1d_r, otf_1d, fact=4):
        sampling = dim / rmax[0]
        ...
        # return np.sum(weights * (1 - np.abs(otf_1d / otf_th_1d_n)))

    pupil  = aperture.disc(1024, 1024, diameter=True, strict=True, cpix=True)
    rmax_init = otf_corr_1d_r[otf_corr_1d >= ythreshold].max()
    p = optimize.minimize(sampling_optim, [rmax_init], args=(pupil, dim, otf_corr_1d_r, otf_corr_1d), 
